...
psi=pi/2.*[0/90,40/90,20/90,0];
theta=pi/2.*[0/90,40/90,20/90,0];
phi=pi/2.*[0/90,40/90,20/90,0];
if (casen==22) || (casen==7)
...
% weathering rate 
 wrct=10^(-12.32)*(t/12)^(-0.603)*100; %manually times 1000
...
pS=cell(phasen,1);
for i=1:phasen
    pC{i}=matrixc_orth(bm(i),sm(i));
%     if i<10
%     pC{i}(1:3,1:3,1:3,1:3) = 0;
%     end
end
pC{phasen}=matrixc_orth(bm(phasen),sm(phasen));
% pC{3}(1,1,1,1)=20;
% pC{3}(1,1,2,2)=6;
% pC{3}(2,2,1,1)=6;
% pC{3}(1,1,3,3)=7;
